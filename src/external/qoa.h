...
void *qoa_encode(const short *sample_data, qoa_desc *qoa, unsigned int *out_len) {
	...
	unsigned char *bytes = static_cast<unsigned char *>(QOA_MALLOC(encoded_size));
...
short *qoa_decode(const unsigned char *bytes, int size, qoa_desc *qoa) {
	...
	short *sample_data = static_cast<short*>(QOA_MALLOC(total_samples * sizeof(short)));
...
void *qoa_read(const char *filename, qoa_desc *qoa) {
	...
	sample_data = qoa_decode(static_cast<const unsigned char *>(data), bytes_read, qoa);
	QOA_FREE(data);
	return sample_data;
}
