...
Model LoadModel(const char *fileName)
{
    ...
    else TRACELOG(TraceLogLevel::LOG_WARNING, "MESH: [%s] Failed to load model mesh(es) data", fileName);

    if (model.materialCount == 0)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MATERIAL: [%s] Failed to load model material data, default to white material", fileName);
...
bool IsModelValid(Model model)
{
    ...
    {
        if ((model.meshes[i].vertices != nullptr) && (model.meshes[i].vboId[0] == 0)) { result = false; break; }  // Vertex position buffer not uploaded to GPU
        if ((model.meshes[i].texcoords != nullptr) && (model.meshes[i].vboId[1] == 0)) { result = false; break; }  // Vertex textcoords buffer not uploaded to GPU
        if ((model.meshes[i].normals != nullptr) && (model.meshes[i].vboId[2] == 0)) { result = false; break; }  // Vertex normals buffer not uploaded to GPU
        if ((model.meshes[i].colors != nullptr) && (model.meshes[i].vboId[3] == 0)) { result = false; break; }  // Vertex colors buffer not uploaded to GPU
        if ((model.meshes[i].tangents != nullptr) && (model.meshes[i].vboId[4] == 0)) { result = false; break; }  // Vertex tangents buffer not uploaded to GPU
        if ((model.meshes[i].texcoords2 != nullptr) && (model.meshes[i].vboId[5] == 0)) { result = false; break; }  // Vertex texcoords2 buffer not uploaded to GPU
        if ((model.meshes[i].indices != nullptr) && (model.meshes[i].vboId[6] == 0)) { result = false; break; }  // Vertex indices buffer not uploaded to GPU
        if ((model.meshes[i].boneIds != nullptr) && (model.meshes[i].vboId[7] == 0)) { result = false; break; }  // Vertex boneIds buffer not uploaded to GPU
        if ((model.meshes[i].boneWeights != nullptr) && (model.meshes[i].vboId[8] == 0)) { result = false; break; }  // Vertex boneWeights buffer not uploaded to GPU
...
void DrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    rlPushMatrix();
        ...
        if (mesh.indices != nullptr) rlDrawVertexArrayElements(0, mesh.triangleCount*3, mesh.indices);
...
void UnloadMesh(Mesh mesh)
{
    ...
    if (mesh.vboId != nullptr) for (int i = 0; i < MAX_MESH_VERTEX_BUFFERS; i++) rlUnloadVertexBuffer(mesh.vboId[i]);
    RL_FREE(mesh.vboId);
...
bool ExportMesh(Mesh mesh, const char *fileName)
{
    ...
    if (IsFileExtension(fileName, ".obj"))
    {
        ...
        int dataSize = vc*(int)strlen("v -0000.000000f -0000.000000f -0000.000000f\n") +
                       ...
                       mesh.triangleCount*snprintf(nullptr, 0, "f %i/%i/%i %i/%i/%i %i/%i/%i\n", vc, vc, vc, vc, vc, vc, vc, vc, vc);
        ...
        // NOTE: Text data length exported is determined by '\0' (nullptr) character
...
bool ExportMeshAsCode(Mesh mesh, const char *fileName)
{
    ...
    //-----------------------------------------------------------------------------------------
    if (mesh.vertices != nullptr)      // Vertex position (XYZ - 3 components per vertex - float)
    {
    ...
    if (mesh.texcoords != nullptr)      // Vertex texture coordinates (UV - 2 components per vertex - float)
    {
    ...
    if (mesh.texcoords2 != nullptr)      // Vertex texture coordinates (UV - 2 components per vertex - float)
    {
    ...
    if (mesh.normals != nullptr)      // Vertex normals (XYZ - 3 components per vertex - float)
    {
    ...
    if (mesh.tangents != nullptr)      // Vertex tangents (XYZW - 4 components per vertex - float)
    {
    ...
    if (mesh.colors != nullptr)        // Vertex colors (RGBA - 4 components per vertex - unsigned char)
    {
    ...
    if (mesh.indices != nullptr)       // Vertex indices (3 index per triangle - unsigned short)
    {
...
static void ProcessMaterialsOBJ(Material *materials, tinyobj_material_t *mats, int materialCount)
{
    ...
    for (int m = 0; m < materialCount; m++)
    {
        ...
        if (mats[m].specular_texname != nullptr) materials[m].maps[MATERIAL_MAP_SPECULAR].texture = LoadTexture(mats[m].specular_texname);  //char *specular_texname; // map_Ks
        ...
        if (mats[m].bump_texname != nullptr) materials[m].maps[std::to_underlying(MaterialMapIndex::MATERIAL_MAP_NORMAL)].texture = LoadTexture(mats[m].bump_texname);  //char *bump_texname; // map_bump, bump
        ...
        if (mats[m].displacement_texname != nullptr) materials[m].maps[std::to_underlying(MaterialMapIndex::MATERIAL_MAP_HEIGHT)].texture = LoadTexture(mats[m].displacement_texname);  //char *displacement_texname; // disp
    }
}
#endif
...
void UpdateModelAnimation(Model model, ModelAnimation anim, int frame)
{
    ...
    for (int m = 0; m < model.meshCount; m++)
    {
        ...
        if (updated)
        {
            ...
            if (mesh.normals != nullptr) rlUpdateVertexBuffer(mesh.vboId[2], mesh.animNormals, mesh.vertexCount*3*sizeof(float), 0); // Update vertex normals
        }
    }
}
...
void GenMeshTangents(Mesh *mesh)
{
    ...
    if ((mesh == nullptr) || (mesh->vertices == nullptr) || (mesh->texcoords == nullptr) || (mesh->normals == nullptr))
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MESH: Tangents generation requires vertices, texcoords and normals vertex attribute data");
        return;
    }

    // Allocate or reallocate tangents data
    if (mesh->tangents == nullptr) mesh->tangents = (float *)RL_MALLOC(mesh->vertexCount*4*sizeof(float));
    else
    {
        RL_FREE(mesh->tangents);
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MESH: Failed to allocate temporary memory for tangent calculation");
...
static void BuildPoseFromParentJoints(BoneInfo *bones, int boneCount, Transform *transforms)
{
    for (int i = 0; i < boneCount; i++)
    {
        if (bones[i].parent >= 0)
        {
            if (bones[i].parent > i)
            {
                TRACELOG(TraceLogLevel::LOG_WARNING, "Assumes bones are toplogically sorted, but bone %d has parent %d. Skipping.", i, bones[i].parent);
                continue;
            }
...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileText == nullptr)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Unable to read obj file", fileName);
        return model;
    }
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: Unable to read obj data %s", fileName);
        return model;
    }
    ...
    for (unsigned int faceId = 0; faceId < objAttributes.num_faces; faceId++)
    {
        ...
        for (int f = 0; f < objAttributes.face_num_verts[faceId]; f++)
        {
            ...
            if (objAttributes.normals != nullptr && normalIndex != TINYOBJ_INVALID_INDEX && normalIndex >= 0)
            {
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Failed to change working directory", currentDir);
    }
...
static Model LoadIQM(const char *fileName)
{
    ...
    */
    ...
    IQMVertexArray *va = nullptr;
    IQMJoint *ijoint = nullptr;

    float *vertex = nullptr;
    float *normal = nullptr;
    float *text = nullptr;
    char *blendi = nullptr;
    unsigned char *blendw = nullptr;
    unsigned char *color = nullptr;

    // In case file can not be read, return an empty model
    if (fileDataPtr == nullptr) return model;
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] IQM file version not supported (%i)", fileName, iqmHeader->version);
        UnloadFileData(fileData);
        return model;
    }
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        TRACELOG(TraceLogLevel::LOG_DEBUG, "MODEL: [%s] mesh name (%s), material (%s)", fileName, name, material);
...
static ModelAnimation *LoadModelAnimationsIQM(const char *fileName, int *animCount)
{
    ...
    *animCount = iqmHeader->num_anims;
    ...
    for (unsigned int a = 0; a < iqmHeader->num_anims; a++)
    {
        ...
        TRACELOG(TraceLogLevel::LOG_INFO, "IQM Anim %s", animations[a].name);
...
static Image LoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    ...
    else if ((cgltfImage->buffer_view != nullptr) && (cgltfImage->buffer_view->buffer->data != nullptr))    // Check if image is provided as data buffer
    {
...
static BoneInfo *LoadBoneInfoGLTF(cgltf_skin skin, int *boneCount)
{
    *boneCount = (int)skin.joints_count;
    ...
    for (unsigned int i = 0; i < skin.joints_count; i++)
    {
        ...
        if (node.name != nullptr) strncpy(bones[i].name, node.name, sizeof(bones[i].name) - 1);
...
static Model LoadGLTF(const char *fileName)
{
    ...
    ***********************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        ...
        TRACELOG(TraceLogLevel::LOG_DEBUG, "    > Primitives (triangles only) count based on hierarchy : %i", primitivesCount);
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < mesh->primitives[p].attributes_count; j++)
                {
                    ...
                    if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_position)      // POSITION, vec3, float
                    {
                        ...
                        if (model.meshes[meshIndex].vertices != nullptr) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Vertices attribute data already loaded", fileName);
                        else
                        {
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_normal)   // NORMAL, vec3, float
                    {
                        ...
                        if (model.meshes[meshIndex].normals != nullptr) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Normals attribute data already loaded", fileName);
                        else
                        {
                            if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_32f))
                            {
                            ...
                            else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Normals attribute data format not supported, use vec3 float", fileName);
                        }
                    }
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_tangent)   // TANGENT, vec4, float, w is tangent basis sign
                    {
                        ...
                        if (model.meshes[meshIndex].tangents != nullptr) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Tangents attribute data already loaded", fileName);
                        else
                        {
                            ...
                            else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Tangents attribute data format not supported, use vec4 float", fileName);
                        }
                    }
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_texcoord) // TEXCOORD_n, vec2, float/u8n/u16n
                    {
                        ...
                        if (attribute->type == cgltf_type_vec2)
                        {
                            ...
                            else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Texcoords attribute data format not supported", fileName);
                        }
                        ...
                        {
                            TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] No more than 2 texture coordinates attributes supported", fileName);
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_color)    // COLOR_n, vec3/vec4, float/u8n/u16n
                    {
                        ...
                        if (model.meshes[meshIndex].colors != nullptr) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Colors attribute data already loaded", fileName);
                        else
                        {
                ...
                if ((mesh->primitives[p].indices != nullptr) && (mesh->primitives[p].indices->buffer_view != nullptr))
                {
                    cgltf_accessor *attribute = mesh->primitives[p].indices;

                    model.meshes[meshIndex].triangleCount = (int)attribute->count/3;

                    if (model.meshes[meshIndex].indices != nullptr) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Indices attribute data already loaded", fileName);
                    else
                    {
        ...
        if (data->skins_count > 0)
        {
            ...
            if (data->skins_count > 1) TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] can only load one skin (armature) per model, but gltf skins_count == %i", fileName, data->skins_count);
        }
        ...
        for (unsigned int i = 0; i < data->nodes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < mesh->primitives[p].attributes_count; j++)
                {
                        ...
                        if (attribute->type == cgltf_type_vec4)
                        {
                            ...
                            else if (attribute->component_type == cgltf_component_type_r_16u)
                            {
                                ...
                                for (int b = 0; b < model.meshes[meshIndex].vertexCount*4; b++)
                                {
                                    ...
                                    {
                                        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Joint attribute data format (u16) overflow", fileName);
                            ...
                            else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Joint attribute data format not supported", fileName);
                        }
                        else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Joint attribute data format not supported", fileName);
                    }
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_weights)  // WEIGHTS_n (vec4, u8n/u16n/f32)
                    {
                        ...
                        if (attribute->type == cgltf_type_vec4)
                        {
                            ...
                            else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Joint weight attribute data format not supported, use vec4 float", fileName);
                        }
                        else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Joint weight attribute data format not supported, use vec4 float", fileName);
                    }
                }
                ...
                if (data->skins_count > 0 && !hasJoints && node->parent != nullptr && node->parent->mesh == nullptr)
                {
                    int parentBoneId = -1;
...
static ModelAnimation *LoadModelAnimationsGLTF(const char *fileName, int *animCount)
{
    ...
    if (result != cgltf_result_success)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Failed to load glTF data", fileName);
        *animCount = 0;
        return nullptr;
    }

    result = cgltf_load_buffers(&options, data, fileName);
    if (result != cgltf_result_success) TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] Failed to load animation buffers", fileName);
    ...
    if (result == cgltf_result_success)
    {
            ...
            for (unsigned int i = 0; i < data->animations_count; i++)
            {
                ...
                for (unsigned int j = 0; j < animData.channels_count; j++)
                {
                    ...
                    if (animData.channels[j].sampler->interpolation != cgltf_interpolation_type_max_enum)
                    {
                        ...
                        {
                            TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Unsupported target_path on channel %d's sampler for animation %d. Skipping.", fileName, j, i);
                        }
                    }
                    else TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Invalid interpolation curve encountered for GLTF animation.", fileName);
                    ...
                    if (!r)
                    {
                        TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Failed to load input time", fileName);
                        continue;
                    }
                ...
                if (animData.name != nullptr) strncpy(animations[i].name, animData.name, sizeof(animations[i].name) - 1);
                ...
                for (int j = 0; j < animations[i].frameCount; j++)
                {
                    ...
                    for (int k = 0; k < animations[i].boneCount; k++)
                    {
                        ...
                        if (boneChannels[k].translate)
                        {
                            ...
                            {
                                TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] Failed to load translate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].rotate)
                        {
                            ...
                            {
                                TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] Failed to load rotate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].scale)
                        {
                            ...
                            {
                                TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] Failed to load scale pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                ...
                TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] Loaded animation: %s (%d frames, %fs)", fileName, (animData.name != nullptr)? animData.name : "nullptr", animations[i].frameCount, animDuration);
        ...
        {
            TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] expected exactly one skin to load animation data from, but found %i", fileName, data->skins_count);
        }
...
static Model LoadM3D(const char *fileName)
{
    ...
    if (fileData != nullptr)
    {
        m3d = m3d_load(fileData, m3d_loaderhook, m3d_freehook, nullptr);

        if (!m3d || M3D_ERR_ISFATAL(m3d->errcode))
        {
            TRACELOG(TraceLogLevel::LOG_WARNING, "MODEL: [%s] Failed to load M3D data, error code %d", fileName, m3d? m3d->errcode : -2);
            if (m3d) m3d_free(m3d);
            UnloadFileData(fileData);
            return model;
        }
        else TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] M3D data loaded successfully: %i faces/%i materials", fileName, m3d->numface, m3d->nummaterial);
        ...
        if (m3d->nummaterial > 0)
        {
            ...
            TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: model has %i material meshes", model.materialCount);
        }
        else
        {
        ...
        */
        ...
        for (i = l = 0, k = -1; i < (int)m3d->numface; i++, l++)
        {
            ...
            if (mi != m3d->face[i].materialid)
            {
                ...
                if ((mi == M3D_UNDEF) && (model.meshes[k].colors != nullptr))
                {
            ...
            if (model.meshes[k].colors != nullptr)
            {
...
static ModelAnimation *LoadModelAnimationsM3D(const char *fileName, int *animCount)
{
    ...
    *animCount = 0;
    ...
    if (fileData != nullptr)
    {
        ...
        else TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] M3D data loaded successfully: %i animations, %i bones, %i skins", fileName,
        ...
        *animCount = m3d->numaction;
        ...
        for (unsigned int a = 0; a < m3d->numaction; a++)
        {
            ...
            TRACELOG(TraceLogLevel::LOG_INFO, "MODEL: [%s] animation #%i: %i msec, %i frames", fileName, a, m3d->action[a].durationmsec, animations[a].frameCount);
