...
static void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    ...
    if (((static_cast<KeyboardKey>(key) == KeyboardKey::KEY_CAPS_LOCK) && ((mods & GLFW_MOD_CAPS_LOCK) > 0)) ||
        ((static_cast<KeyboardKey>(key) == KeyboardKey::KEY_NUM_LOCK) && ((mods & GLFW_MOD_NUM_LOCK) > 0))) CORE.Input.Keyboard.currentKeyState[key] = 1;
