...
void InitAudioDevice(void)
{
    ...
    ma_result result = ma_context_init(nullptr, 0, &ctxConfig, &AUDIO.System.context);
    if (result != MA_SUCCESS)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to initialize context");
        return;
    }
    ...
    config.playback.pDeviceID = nullptr;  // nullptr for the default playback AUDIO.System.device
    config.playback.format = AUDIO_DEVICE_FORMAT;
    config.playback.channels = AUDIO_DEVICE_CHANNELS;
    config.capture.pDeviceID = nullptr;  // nullptr for the default capture AUDIO.System.device
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to initialize playback device");
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to create mutex for mixing");
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to start playback device");
        ma_device_uninit(&AUDIO.System.device);
        ma_context_uninit(&AUDIO.System.context);
        return;
    }

    TRACELOG(TraceLogLevel::LOG_INFO, "AUDIO: Device initialized successfully");
    TRACELOG(TraceLogLevel::LOG_INFO, "    > Backend:       miniaudio | %s", ma_get_backend_name(AUDIO.System.context.backend));
    TRACELOG(TraceLogLevel::LOG_INFO, "    > Format:        %s -> %s", ma_get_format_name(AUDIO.System.device.playback.format), ma_get_format_name(AUDIO.System.device.playback.internalFormat));
    TRACELOG(TraceLogLevel::LOG_INFO, "    > Channels:      %d -> %d", AUDIO.System.device.playback.channels, AUDIO.System.device.playback.internalChannels);
    TRACELOG(TraceLogLevel::LOG_INFO, "    > Sample rate:   %d -> %d", AUDIO.System.device.sampleRate, AUDIO.System.device.playback.internalSampleRate);
    TRACELOG(TraceLogLevel::LOG_INFO, "    > Periods size:  %d", AUDIO.System.device.playback.internalPeriodSizeInFrames*AUDIO.System.device.playback.internalPeriods);
...
void CloseAudioDevice(void)
{
    if (AUDIO.System.isReady)
    {
        ...
        AUDIO.System.pcmBuffer = nullptr;
        AUDIO.System.pcmBufferSize = 0;

        TRACELOG(TraceLogLevel::LOG_INFO, "AUDIO: Device closed successfully");
    }
    else TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Device could not be closed, not currently initialized");
}
...
AudioBuffer *LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage)
{
    ...
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to allocate memory for buffer");
        return nullptr;
    }

    if (sizeInFrames > 0) audioBuffer->data = static_cast<unsigned char *>(RL_CALLOC(sizeInFrames*channels*ma_get_bytes_per_sample(format), 1));
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Failed to create data conversion pipeline");
        RL_FREE(audioBuffer);
        return nullptr;
    }
...
void TrackAudioBuffer(AudioBuffer *buffer)
{
    ma_mutex_lock(&AUDIO.System.lock);
    {
        if (AUDIO.Buffer.first == nullptr) AUDIO.Buffer.first = buffer;
        else
        {
...
void UntrackAudioBuffer(AudioBuffer *buffer)
{
    ma_mutex_lock(&AUDIO.System.lock);
    {
        if (buffer->prev == nullptr) AUDIO.Buffer.first = buffer->next;
        else buffer->prev->next = buffer->next;

        if (buffer->next == nullptr) AUDIO.Buffer.last = buffer->prev;
...
Wave LoadWave(const char *fileName)
{
    ...
    if (fileData != nullptr) wave = LoadWaveFromMemory(GetFileExtension(fileName), fileData, dataSize);
...
Wave LoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    else if ((strcmp(fileType, ".wav") == 0) || (strcmp(fileType, ".WAV") == 0))
    {
        ...
        if (success)
        {
            ...
            drwav_read_pcm_frames_s16(&wav, wave.frameCount, static_cast<drwav_int16*>(wave.data));
        }
        else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to load WAV data");
    ...
    {
        stb_vorbis *oggData = stb_vorbis_open_memory((unsigned char *)fileData, dataSize, nullptr, nullptr);
        ...
        else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to load OGG data");
    }
#endif
#if defined(SUPPORT_FILEFORMAT_MP3)
    else if ((strcmp(fileType, ".mp3") == 0) || (strcmp(fileType, ".MP3") == 0))
    {
        ...
        wave.data = drmp3_open_memory_and_read_pcm_frames_f32(fileData, dataSize, &config, &totalFrameCount, nullptr);
        wave.sampleSize = 32;
        ...
        else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to load MP3 data");
    ...
    else if ((strcmp(fileType, ".qoa") == 0) || (strcmp(fileType, ".QOA") == 0))
    {
        ...
        else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to load QOA data");
    ...
    else if ((strcmp(fileType, ".flac") == 0) || (strcmp(fileType, ".FLAC") == 0))
    {
        ...
        // NOTE: We are forcing conversion to 16bit sample size on reading
        wave.data = drflac_open_memory_and_read_pcm_frames_s16(fileData, dataSize, &wave.channels, &wave.sampleRate, &totalFrameCount, nullptr);
        wave.sampleSize = 16;

        if (wave.data != nullptr) wave.frameCount = (unsigned int)totalFrameCount;
        else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to load FLAC data");
    }
#endif
    else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Data format not supported");

    TRACELOG(TraceLogLevel::LOG_INFO, "WAVE: Data loaded successfully (%i Hz, %i bit, %i channels)", wave.sampleRate, wave.sampleSize, wave.channels);
...
bool IsWaveValid(Wave wave)
{
    ...
    if ((wave.data != nullptr) &&      // Validate wave data available
...
Sound LoadSoundFromWave(Wave wave)
{
    ...
    if (wave.data != nullptr)
    {
        ...
        ma_uint32 frameCount = (ma_uint32)ma_convert_frames(nullptr, 0, AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, nullptr, frameCountIn, formatIn, wave.channels, wave.sampleRate);
        if (frameCount == 0) TRACELOG(TraceLogLevel::LOG_WARNING, "SOUND: Failed to get frame count for format conversion");
        ...
        if (audioBuffer == nullptr)
        {
            TRACELOG(TraceLogLevel::LOG_WARNING, "SOUND: Failed to create buffer");
        ...
        if (frameCount == 0) TRACELOG(TraceLogLevel::LOG_WARNING, "SOUND: Failed format conversion");
...
bool IsSoundValid(Sound sound)
{
    ...
    if ((sound.frameCount > 0) &&           // Validate frame count
        (sound.stream.buffer != nullptr) &&    // Validate stream buffer
...
{
    RL_FREE(wave.data);
    //TRACELOG(TraceLogLevel::LOG_INFO, "WAVE: Unloaded wave data from RAM");
}
...
void UnloadSound(Sound sound)
{
    ...
    //TRACELOG(TraceLogLevel::LOG_INFO, "SOUND: Unloaded sound data from RAM");
}

void UnloadSoundAlias(Sound alias)
{
    // Untrack and unload just the sound buffer, not the sample data, it is shared with the source for the alias
    if (alias.stream.buffer != nullptr)
    {
        UntrackAudioBuffer(alias.stream.buffer);
        ma_data_converter_uninit(&alias.stream.buffer->converter, nullptr);
...
bool ExportWave(Wave wave, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".wav"))
    {
        ...
        success = drwav_init_memory_write(&wav, &fileData, &fileDataSize, &format, nullptr);
    ...
    else if (IsFileExtension(fileName, ".qoa"))
    {
        ...
        else TRACELOG(TraceLogLevel::LOG_WARNING, "AUDIO: Wave data must be 16 bit per sample for QOA format export");
    }
#endif
    ...
    if (success) TRACELOG(TraceLogLevel::LOG_INFO, "FILEIO: [%s] Wave data exported successfully", fileName);
    else TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: [%s] Failed to export wave data", fileName);
...
bool ExportWaveAsCode(Wave wave, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(TraceLogLevel::LOG_INFO, "FILEIO: [%s] Wave as code exported successfully", fileName);
    else TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: [%s] Failed to export wave as code", fileName);
...
void WaveFormat(Wave *wave, int sampleRate, int sampleSize, int channels)
{
    ...
    ma_uint32 frameCountIn = wave->frameCount;
    ma_uint32 frameCount = (ma_uint32)ma_convert_frames(nullptr, 0, formatOut, channels, sampleRate, nullptr, frameCountIn, formatIn, wave->channels, wave->sampleRate);

    if (frameCount == 0)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed to get frame count for format conversion");
        return;
    }
    ...
    {
        RL_FREE(wave->data);
        TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Failed format conversion");
        return;
    }
...
void WaveCrop(Wave *wave, int initFrame, int finalFrame)
{
    ...
    else TRACELOG(TraceLogLevel::LOG_WARNING, "WAVE: Crop range out of bounds");
}
...
Music LoadMusicStream(const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".wav"))
    {
        ...
        bool success = drwav_init_file(ctxWav, fileName, nullptr);
    ...
    else if (IsFileExtension(fileName, ".ogg"))
    {
        ...
        stb_vorbis *ctxOgg = stb_vorbis_open_filename(fileName, nullptr, nullptr);
    ...
    else if (IsFileExtension(fileName, ".mp3"))
    {
        ...
        int result = drmp3_init_file(ctxMp3, fileName, nullptr);
    ...
    {
        drflac *ctxFlac = drflac_open_file(fileName, nullptr);
    ...
    {
        jar_xm_context_t *ctxXm = nullptr;
...
#endif
    else TRACELOG(TraceLogLevel::LOG_WARNING, "STREAM: [%s] File format not supported", fileName);

    if (!musicLoaded)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: [%s] Music file could not be opened", fileName);
    }
    else
    {
        // Show some music stream info
        TRACELOG(TraceLogLevel::LOG_INFO, "FILEIO: [%s] Music file loaded successfully", fileName);
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Total frames:  %i", music.frameCount);
    }
...
Music LoadMusicStreamFromMemory(const char *fileType, const unsigned char *data, int dataSize)
{
    ...
    else if ((strcmp(fileType, ".wav") == 0) || (strcmp(fileType, ".WAV") == 0))
    {
        ...
        bool success = drwav_init_memory(ctxWav, (const void *)data, dataSize, nullptr);
    ...
    {
        // Open ogg audio stream
        stb_vorbis *ctxOgg = stb_vorbis_open_memory((const unsigned char *)data, dataSize, nullptr, nullptr);
    ...
    {
        drmp3 *ctxMp3 = (drmp3 *)RL_CALLOC(1, sizeof(drmp3));
        int success = drmp3_init_memory(ctxMp3, (const void*)data, dataSize, nullptr);
    ...
    {
        drflac *ctxFlac = drflac_open_memory((const void*)data, dataSize, nullptr);
...
#endif
    else TRACELOG(TraceLogLevel::LOG_WARNING, "STREAM: Data format not supported");

    if (!musicLoaded)
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: Music data could not be loaded");
    }
    else
    {
        // Show some music stream info
        TRACELOG(TraceLogLevel::LOG_INFO, "FILEIO: Music data loaded successfully");
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(TraceLogLevel::LOG_INFO, "    > Total frames:  %i", music.frameCount);
...
{
    return ((music.ctxData != nullptr) &&          // Validate context loaded
...
#endif
#if defined(SUPPORT_FILEFORMAT_FLAC)
        else if (music.ctxType == MUSIC_AUDIO_FLAC) { drflac_close((drflac *)music.ctxData); drflac_free((drflac *)music.ctxData, nullptr); }
#endif
#if defined(SUPPORT_FILEFORMAT_XM)
...
float GetMusicTimePlayed(Music music)
{
    ...
    if (music.stream.buffer != nullptr)
    {
        ...
        if (music.ctxType == MUSIC_MODULE_XM)
        {
            ...
            jar_xm_get_position(static_cast<jar_xm_context_t *>(music.ctxData), nullptr, nullptr, nullptr, &framesPlayed);
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    if (stream.buffer != nullptr)
    {
        ...
        TRACELOG(TraceLogLevel::LOG_INFO, "STREAM: Initialized successfully (%i Hz, %i bit, %s)", stream.sampleRate, stream.sampleSize, (stream.channels == 1)? "Mono" : "Stereo");
    }
    else TRACELOG(TraceLogLevel::LOG_WARNING, "STREAM: Failed to load audio buffer, stream could not be created");
...
{
    return ((stream.buffer != nullptr) &&    // Validate stream buffer
...
void UnloadAudioStream(AudioStream stream)
{
    ...
    TRACELOG(TraceLogLevel::LOG_INFO, "STREAM: Unloaded audio stream data from RAM");
}
...
static void OnLog(void *pUserData, ma_uint32 level, const char *pMessage)
{
    TRACELOG(TraceLogLevel::LOG_WARNING, "miniaudio: %s", pMessage);   // All log messages from miniaudio are errors
}
...
static void OnSendAudioDataToDevice(ma_device *pDevice, void *pFramesOut, const void *pFramesInput, ma_uint32 frameCount)
{
    ...
    ma_mutex_lock(&AUDIO.System.lock);
    {
        for (AudioBuffer *audioBuffer = AUDIO.Buffer.first; audioBuffer != nullptr; audioBuffer = audioBuffer->next)
        {
...
static bool IsAudioBufferPlayingInLockedState(AudioBuffer *buffer)
{
    ...
    if (buffer != nullptr) result = (buffer->playing && !buffer->paused);
...
static void UpdateAudioStreamInLockedState(AudioStream stream, const void *data, int frameCount)
{
    if (stream.buffer != nullptr)
    {
        if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1])
        {
            ...
            else TRACELOG(TraceLogLevel::LOG_WARNING, "STREAM: Attempting to write too many frames to buffer");
        }
        else TRACELOG(TraceLogLevel::LOG_WARNING, "STREAM: Buffer not available for updating");
    }
}
...
static bool SaveFileData(const char *fileName, void *data, int dataSize)
{
    if (fileName != nullptr)
    {
        ...
        {
            TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: [%s] Failed to open file", fileName);
            return false;
        }
    }
    else
    {
        TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: File name provided is not valid");
        return false;
    }
...
static bool SaveFileText(const char *fileName, char *text)
{
    if (fileName != nullptr)
    {
        ...
        {
            TRACELOG(TraceLogLevel::LOG_WARNING, "FILEIO: [%s] Failed to open text file", fileName);
            return false;
        }
    }
    else
